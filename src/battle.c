...
void battle_rewards(Player* player, Supemon* foe)
{
    ...
    printf("%s%s%s gained ", COLOR_PLAYER, active->name, RESET);
    ...
    switch (choice)
    {
        case 1:
        {
            ...
            for (move_count = 0; move_count < MAX_MOVES; move_count++)
            {
                ...
                // Colorier selon le type de move
                const char* move_color;
                switch(move->type) {
                    case MOVE_OFFENSIVE:
                        move_color = COLOR_DAMAGE;
                        break;
                    case MOVE_ATTACK:
                    case MOVE_DEFENSE:
                    case MOVE_EVASION:
                    case MOVE_ACCURACY:
                        move_color = COLOR_STAT_UP;
                        break;
                    default:
                        move_color = RESET;
                }

                printf("| %s%d - %-24s%s|\n", move_color, move_count + 1, move->name, RESET);
            }
            ...
            clear_terminal();
            printf("%s%s%s fainted. ", COLOR_ENEMY, foe->name, RESET);
            printf("%s%s%s %swon the fight!%s\n", COLOR_PLAYER, active->name, RESET, COLOR_SUCCESS, RESET);
        ...
        case 4:
        {
            ...
            if (rnd <= rate)
            {
                ...
                printf("%sSuccessfully captured%s %s%s%s!\n", COLOR_SUCCESS, RESET, COLOR_ENEMY, foe->name, RESET);
                battle_rewards(player, foe);
        ...
        case 5:
        {
            ...
            {
                clear_terminal();
                print_colored(COLOR_WARNING, "You ran away !\n\n");
                npc_dialog("...", 1000);
...
int foe_turn(Player* player, Supemon* foe)
{
    ...
    clear_terminal();
    printf("%s%s%s fainted. ", COLOR_PLAYER, active->name, RESET);
